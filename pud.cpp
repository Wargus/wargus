...
const std::string RaceNames[] = {"human", "orc", "neutral"};

const std::string TilesetTypeStrings[] = {"summer", "winter", "wasteland",
                                          "swamp"
                                         };

// unit script names that correspond to the unit types
const std::string UnitScriptNames[] = {"unit-footman",
                                       "unit-grunt",
                                       "unit-peasant",
                                       "unit-peon",
                                       "unit-ballista",
...
void PudData::WriteSMP(gzFile smpout, const fs::path &smsname) const
{
    ...
    gzprintf(smpout,
             "-- File generated automatically from pudconvert V" VERSION "\n");
    gzprintf(smpout, "\n");
    ...
    if (!smsname.empty()) {
        buf.clear();
        buf = smsname.string();
        if (buf.find("./campaigns") != std::string::npos) {
            auto ind = buf.find("./campaigns");
            buf.erase(0, ind);
        }
...
void PudData::WriteSMS(gzFile smsout) const
{
    ...
    for (int i = 0; i != PLAYERMAX - 1; ++i) {
        ...
        gzprintf(smsout, "SetPlayerData(%d, \"RaceName\", \"%s\")\n", i,
                 RaceNames[pdata.Races[i]].c_str());
        if (pdata.Players[i] != PlayerNeutral) {
            gzprintf(smsout, "SetAiType(%d, \"%s\")\n", i,
                     AiTypeNames[pdata.AiType[i]].c_str());
        }
    }
    ...
    for (int y = 0; y < pdata.MapSizeY; ++y) {
        for (int x = 0; x < pdata.MapSizeX; ++x) {
            ...
            gzprintf(smsout, "SetTile(%d, %d, %d, %d)\n", pdata.Tiles[mapOffset], x,
                     y, pdata.Value[mapOffset]);
        }
    }
    ...
    for (size_t i = 0; i < pdata.Units.size(); ++i) {
        ...
        if (UnitScriptNames[pdata.Units[i].Type].size() <= 1) {
            fprintf(stderr,
                    "Skipping unknown unit type %d for player %d at [%d, %d]\n",
                    pdata.Units[i].Type, pdata.Units[i].Player, pdata.Units[i].X,
                    pdata.Units[i].Y);
            continue;
        }
        gzprintf(smsout, "unit = CreateUnit(\"%s\", %d, {%d, %d})\n",
                 UnitScriptNames[pdata.Units[i].Type].c_str(),
                 pdata.Units[i].Player, pdata.Units[i].X, pdata.Units[i].Y);
        if (pdata.Units[i].Type == UnitHumanOilPlatform ||
            pdata.Units[i].Type == UnitOrcOilPlatform ||
            pdata.Units[i].Type == UnitGoldMine ||
            pdata.Units[i].Type == UnitOilPatch) {
        ...
        if (!strcmp(header, "VER ")) {
            ...
            for (int j = 0; j < pdata.MapSizeY; ++j) {
                for (int i = 0; i < pdata.MapSizeX; ++i) {
                    int v = curp[j * pdata.MapSizeX * 2 + i * 2] |
                            (curp[j * pdata.MapSizeX * 2 + i * 2 + 1] << 8);
                    pdata.Tiles[j * pdata.MapSizeX + i] = v;
                    if ((v & 0xFFF0) == 0x00A0 || (v & 0xFFF0) == 0x00C0 ||
                        (v & 0xFF00) == 0x0900) {
                        // orc wall
                        pdata.Value[j * pdata.MapSizeX + i] = 40;
                    } else if ((v & 0x00F0) == 0x0090 || (v & 0xFFF0) == 0x00B0 ||
                               (v & 0xFF00) == 0x0800) {
                        // human wall
                        pdata.Value[j * pdata.MapSizeX + i] = 40;
                    } else {
...
int PudToStratagus(const unsigned char *puddata, size_t size,
                   const fs::path &name, const fs::path &outdir)
{
    PudData pdata;
