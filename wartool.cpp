                   ...
                   const fs::path &szFileName, bool compress)
{
    ...
    if (nError == ERROR_SUCCESS) {
        if (!SFileOpenArchive(szArchiveName.c_str(), 0, STREAM_FLAG_READ_ONLY, &hMpq)) {
            nError = GetLastError();
        }
    }
...
int ConvertRgb(const fs::path &file, int rgbe)
{
    ...
    CheckPath(buf);
    {
        ...
        f << "GIMP Palette\n# Stratagus " << toupper(file.c_str()[0]) << file.c_str() + 1 << " -- GIMP Palette file\n";

        for (i = 0; i < 256; ++i) {
            // FIXME: insert nice names!
            f << rgbp[i * 3] << " " << rgbp[i * 3 + 1] << " " << rgbp[i * 3 + 2] << '\t' << '#' << i << "\n";
        }
    }
...
#ifdef WIN32
        int execresult = _wexecv(argv[0], argv);
#else
...
void teeStdout()
{
    ...
    char *stdoutpath = GetExtractionLogPath("Wargus", (char *)Dir.c_str());
    int logfilefd =
    ...
    for (u = 0; u < sizeof(Todo) / sizeof(*Todo); ++u) {
        ...
        switch (Todo[u].Type) {
            ...
            case Q:
                ...
                if (ExtractMPQFile(mpqfile, Todo[u].ArcFile,
                                   extract, Todo[u].Arg2 == 1)) {
