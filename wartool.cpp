...
#ifdef USE_STORMLIB
int ExtractMPQFile(const char *szArchiveName, char *szArchivedFile,
                   const char *szFileName, bool compress) {
...
void ConvertPud(const fs::path file, int pude, bool justconvert = false) {
  ...
  if (justconvert == false) {
    ...
    PudToStratagus(
        &pudp[0], filesize,
        /*strrchr(file, '/') + 1--Similar to above*/ file.filename().c_str(),
        buf.c_str());
    fs::remove(pudfile);
  }
}
...
int ConvertVideo(const char *file, int video, bool justconvert = false) {
  ...
  cmd += buf;
  cmd += '"';
  cmd += " -codec:v libtheora -qscale:v 31 -codec:a libvorbis -qscale:a 15 "
         "-pix_fmt yuv420p ";
  if (CDType & CD_BNE) {
...
void FixTranslation(const fs::path &translation) {
  if (fs::exists(translation)) {
    ...
    std::unique_ptr<unsigned char[]> buf =
        std::make_unique<unsigned char[]>(filesize);
...
void teeStdout() {
  ...
  int logfilefd =
      _open(stdoutpath, _O_WRONLY | _O_CREAT | _O_BINARY | _O_TRUNC, _S_IWRITE);
...
int main(int argc, char **argv) {
  ...
  } else if (buf = ArchiveDir + std::string("/Support/TOMES/TOME.1");
             fs::exists(buf)) {
    filename = ArchiveDir;
    filename /= "/Support/TOMES/TOME.4";
    ...
    case Q:
      ...
      if (Todo[u].Arg1 == 1) { // local archive
        ...
        if (!fs::exists(mpqfile)) {
          ...
          error(mpqfile, "I also tried different cases and tried both .mpq and "
                         ".exe extensions\n");
        }
      ...
      if (ExtractMPQFile(mpqfile.c_str(), (char *)Todo[u].ArcFile,
                         extract.c_str(), Todo[u].Arg2 == 1)) {
